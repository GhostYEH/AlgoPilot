...
class ResourceGenerationWorkflow:
    ...
    async def run(
        ...
        *,
        ...
        await _emit("safety_filter", "SafetyAgent", "running", "内容安全审查中…")

        safe_text, safety_logs, safety_ok = safety_agent.audit(content, resource_type=resource_type)

        for slog in safety_logs:
            ctx.log(
                slog["agent"],
                slog["action"],
                slog.get("detail", ""),
                resource_type=resource_type,
                status=slog.get("status", "done"),
            )

        if not safety_ok:
            detail = safety_logs[-1].get("detail", "内容未通过安全审查") if safety_logs else "未通过"
            await _emit("safety_filter", "SafetyAgent", "error", detail)
            raise ValueError(detail)

        warn_logs = [s for s in safety_logs if s.get("status") == "warn"]
        gen_meta["safety_warnings"] = [w.get("detail", "") for w in warn_logs if w.get("detail")]
        gen_meta["agent_logs"] = list(ctx.agent_logs)
        detail = safety_logs[-1].get("detail", "安全审查通过") if safety_logs else "安全审查通过"
        await _emit("safety_filter", "SafetyAgent", "done", detail)

        await _emit("persist", "Orchestrator", "done" if verify_passed else "warn", gen_meta.get("status", ""))
