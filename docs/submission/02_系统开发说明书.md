...
> 文档版本：v1.1 · 日期：2026-07-12 · 对应代码版本：main 分支
...
│  30 余个页面 · 70 组件 · 13 个算法游戏 · 13 个 Trace 组件  │
└────────────────────────┬────────────────────────────────┘
                         │ HTTP / SSE / Axios
┌────────────────────────▼────────────────────────────────┐
│                   后端（FastAPI + Python）                │
...
| **数据层** | SQLAlchemy 2.0 + SQLite/MySQL | 7 张表、外键级联、JSON 字段、会话管理 |
...
**与 LangChain/LangGraph 的关系**：系统借鉴状态图与 DAG 编排思想，自主实现轻量级多智能体工作流（零 langgraph 依赖、易部署）。项目未使用 LangChain、LangGraph 或 OpenAI SDK，编排通过 Python `asyncio` + `contextvars` 实现。
...
#### 4.4.2 批量五类资源（generate-all）四阶段拓扑
